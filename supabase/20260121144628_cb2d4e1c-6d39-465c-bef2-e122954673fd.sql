-- Update RLS policies for data_room_document_content to include file-level permission checks

-- Drop existing policies
DROP POLICY IF EXISTS "Users can view data room document content" ON public.data_room_document_content;
DROP POLICY IF EXISTS "Users can create data room document content" ON public.data_room_document_content;
DROP POLICY IF EXISTS "Users can update data room document content" ON public.data_room_document_content;
...
USING (
  ...
  EXISTS (
    ...
    WHERE data_rooms.id = data_room_document_content.data_room_id
  ...
  EXISTS (
    ...
    WHERE data_room_file_permissions.file_id = data_room_document_content.file_id
...
WITH CHECK (
  ...
  OR
  -- Users with file-level edit permission can create
  EXISTS (
    SELECT 1 FROM data_room_file_permissions
    WHERE data_room_file_permissions.file_id = data_room_document_content.file_id
    AND data_room_file_permissions.user_id = auth.uid()
...
-- Create updated UPDATE policy that also checks file permissions
CREATE POLICY "Users can update data room document content"
ON public.data_room_document_content
FOR UPDATE
USING (
  -- Room members can update
  EXISTS (
    SELECT 1 FROM data_room_members
    WHERE data_room_members.data_room_id = data_room_document_content.data_room_id
    AND data_room_members.user_id = auth.uid()
  ...
  OR
  -- Users with file-level edit permission can update
  EXISTS (
    SELECT 1 FROM data_room_file_permissions
    WHERE data_room_file_permissions.file_id = data_room_document_content.file_id
    AND data_room_file_permissions.user_id = auth.uid()
