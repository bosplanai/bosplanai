...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Verifying dataroom storage subscription:", { sessionId, userId: user.id });
    ...
    if ((metadataType !== "storage_subscription" && metadataType !== "storage_purchase") || session.metadata?.product !== "dataroom") {
    ...
    if (existingPurchase) {
      ...
      return new Response(
        JSON.stringify({ success: true, message: "Subscription already processed", storageGb, storageMb }),
    ...
    const { error: purchaseError } = await supabase.from("dataroom_storage_purchases").insert({
      ...
      price_id: typeof subscription === 'object' ? subscription.id : "monthly-dataroom-storage-subscription",
    ...
    console.log("Dataroom storage subscription verified and applied:", {
      organizationId,
      addedGb: storageGb,
    ...
    console.error("Error verifying dataroom storage subscription:", error);
