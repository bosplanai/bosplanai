...
CREATE TABLE IF NOT EXISTS public.task_merge_logs (
...
-- Org members can view merge logs in their organization
...
CREATE INDEX IF NOT EXISTS idx_task_merge_logs_org ON public.task_merge_logs(organization_id);
CREATE INDEX IF NOT EXISTS idx_task_merge_logs_status ON public.task_merge_logs(status) WHERE status = 'pending_revert';