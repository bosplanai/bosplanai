...
export function DataRoomEditDetailsDialog({
  ...
  const toggleGuest = (guestId: string) => {
    setAssignedGuests(prev => prev.includes(guestId) ? prev.filter(id => id !== guestId) : [...prev, guestId]);
  };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3 p-4 rounded-lg bg-muted/50 border border-border">
              ...
              {members.length > 0 && <div className="space-y-2">
                  ...
                  <div className="max-h-24 overflow-y-auto space-y-1">
                    {members.map(member => <label key={member.id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-muted/50 p-1 rounded">
                        <Checkbox checked={assignedUsers.includes(member.id)} onCheckedChange={() => toggleUser(member.id)} />
              ...
              {activeGuests.length > 0 && <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground">External Guests</p>
                  <div className="max-h-24 overflow-y-auto space-y-1">
                    {activeGuests.map(guest => <label key={guest.id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-muted/50 p-1 rounded">
                        <Checkbox checked={assignedGuests.includes(guest.id)} onCheckedChange={() => toggleGuest(guest.id)} />
              ...
              {members.length === 0 && activeGuests.length === 0 && <p className="text-xs text-muted-foreground">
                  No team members or guests available
                </p>}

              {/* Selected badges */}
              {(assignedUsers.length > 0 || assignedGuests.length > 0) && <div className="flex flex-wrap gap-1 pt-1">
                  {assignedUsers.map(userId => {
              const member = members.find(m => m.id === userId);
              return <Badge key={userId} variant="secondary" className="text-xs gap-1">
                        {member?.full_name}
                        <X className="w-3 h-3 cursor-pointer hover:text-destructive" onClick={() => toggleUser(userId)} />
                      </Badge>;
            })}
                  {assignedGuests.map(guestId => {
              const guest = activeGuests.find(g => g.id === guestId);
              return <Badge key={guestId} variant="outline" className="text-xs gap-1 bg-blue-500/10 text-blue-600 border-blue-500/20">
                        <Globe className="w-3 h-3" />
                        {guest?.guest_name || guest?.email}
                        <X className="w-3 h-3 cursor-pointer hover:text-destructive" onClick={() => toggleGuest(guestId)} />
                      </Badge>;
            })}
                </div>}
            </div>
        ...
        <DialogFooter>
          ...
          <Button className="bg-emerald-500 hover:bg-emerald-600" onClick={handleSave} disabled={isSaving}>
            Save Changes
          </Button>
        </DialogFooter>
      </DialogContent>
