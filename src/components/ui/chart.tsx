 ...
 * IMPORTANT: This component should only receive trusted configuration from hardcoded chart configs.
 */
const sanitizeCssColor = (color: string): string | null => {
  ...
  // Validate against allowed CSS color patterns
  const validPatterns = [
    ...
    /^var\(--[a-zA-Z0-9-]+\)$/,
    // Named colors (common ones) - strict alphanumeric only
    /^[a-zA-Z]{3,20}$/,
  ];
  ...
  if (!isValid) {
    console.warn(`ChartStyle: Invalid or potentially unsafe color value rejected: ${sanitized.substring(0, 50)}`);
    return null;
  }
...
const sanitizeCssKey = (key: string): string | null => {
  ...
  // Only allow alphanumeric and hyphens for CSS variable names
  const sanitized = key.replace(/[^a-zA-Z0-9-]/g, '');
  
  if (sanitized !== key) {
    console.warn(`ChartStyle: CSS key sanitized from "${key}" to "${sanitized}"`);
  }
