...
import { CreditCard, Download, ExternalLink, FileText, Calendar, Clock } from "lucide-react";
...
const BillingSettingsContent = () => {
  ...
  const fetchSubscriptionAndInvoices = async () => {
    try {
      ...
      const { data: invData, error: invError } = await supabase.functions.invoke('get-invoices');
              ...
              <Table>
                ...
                <TableBody>
                  {invoices.map(invoice => (
                    <TableRow key={invoice.id} className="border-border/40 hover:bg-muted/30 transition-colors duration-200">
                      <TableCell>
                        <div>
                        ...
                        <div className="flex justify-end gap-1.5">
                          {invoice.hosted_invoice_url && (
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="rounded-lg hover:bg-secondary/80 transition-all duration-200" 
