...
const BOARDS: BoardConfig[] = [
  {
    ...
    examples: [
      "Launch a new e-commerce website with Shopify",
      "Build a mobile app MVP for customer engagement",
  ...
  },
  {
    ...
    examples: [
      "Expand into 2 new markets this year",
      "Develop partnership strategy with key vendors",
      "Create 5-year business growth plan",
    ],
  },
];
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed
        ...
        .map((item) => ({
          title: item.title.trim().slice(0, 100), // Limit title length
  ...
  for (const line of lines) {
    ...
    if (taskText && taskText.length > 3) {
      ...
      if (cleanedText.length > 5 || /[a-z]/.test(cleanedText)) {
        ...
        const title = cleanedText.length > 60 
          ? cleanedText.slice(0, 57) + "..." 
          : cleanedText;
        tasks.push({
          title,
...
const Onboarding = () => {
  ...
  const { organization, profile, refetch } = useOrganization();
  const { user } = useAuth();
  const { canAccessOperational, canAccessStrategic, loading: roleLoading } = useUserRole();
  ...
  const handleUpdateGeneratedTask = useCallback((boardId: string, index: number, field: "title" | "description", value: string) => {
    setGeneratedTasks((prev) => {
      ...
      const next = boardTasks.map((t, i) => (i === index ? { ...t, [field]: value } : t));
  ...
  const totalTasksCount = Object.values(generatedTasks).reduce(
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-secondary/20 to-background flex flex-col">
      ...
      <header className="border-b border-border/60 bg-card/80 backdrop-blur-sm px-4 sm:px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <img src={bosplanLogo} alt="Bosplan" className="h-8 sm:h-10" />
          <Button
            variant="ghost"
            size="sm"
          ...
          <Card className="shadow-md border-border/60 overflow-hidden">
            <CardHeader className="bg-secondary/30 border-b border-border/40">
              <div className="flex items-center gap-3">
                ...
                {completedBoards.has(currentBoard.id) && (
                  <CheckCircle2 className="w-5 h-5 text-brand-green ml-auto" />
                )}
              </div>
            </CardHeader>
            <CardContent className="pt-6 space-y-5">
              ...
              <div className="space-y-3">
                ...
                <Textarea
                  placeholder={currentBoard.placeholder}
                  value={prompts[currentBoard.id]}
                  onChange={(e) =>
                    setPrompts((prev) => ({
                      ...prev,
                  ...
                  className="min-h-[100px] resize-none rounded-xl bg-background/80 border-brand-green focus:border-brand-orange transition-colors"
                ...
                <div className="flex flex-wrap gap-2">
                  {currentBoard.examples.map((example, idx) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="text-xs rounded-full border-dashed hover:border-brand-orange hover:text-brand-orange transition-colors"
                      onClick={() =>
                        setPrompts((prev) => ({
                          ...prev,
              ...
              {isSingleBoardFlow && (
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      variant={managerDestination === "product" ? "default" : "outline"}
                      size="sm"
                      className="rounded-full"
                      onClick={() => setManagerDestination("product")}
                    ...
                    <Button
                      type="button"
                      variant={managerDestination === "personal" ? "default" : "outline"}
                      size="sm"
                      className="rounded-full"
                      onClick={() => setManagerDestination("personal")}
                      disabled={isGenerating || isCreating}
                    >
                      Personal checklist
                    </Button>
              ...
              <Button
                ...
                disabled={isGenerating || !prompts[currentBoard.id].trim()}
                className="w-full bg-brand-orange hover:bg-brand-orange/90 text-white rounded-xl h-11"
              >
                {isGenerating ? (
                  <>
              ...
              {generatedTasks[currentBoard.id].length > 0 && (
                <div className="space-y-3 pt-3 border-t border-border/40">
                  ...
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                    {generatedTasks[currentBoard.id].map((task, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-2 text-sm text-muted-foreground p-3 bg-secondary/50 rounded-lg"
                      >
                        ...
                        <div className="flex-1 space-y-2">
                          <Input
                            value={task.title}
                            onChange={(e) =>
                              handleUpdateGeneratedTask(currentBoard.id, idx, "title", e.target.value)
                            }
                          ...
                          <Textarea
                            ...
                            onChange={(e) =>
                              handleUpdateGeneratedTask(currentBoard.id, idx, "description", e.target.value)
                            }
                        ...
                        <Button
                          ...
                          size="icon"
                          className="shrink-0"
                          onClick={() => handleRemoveGeneratedTask(currentBoard.id, idx)}
                          disabled={isCreating}
            ...
            <div className="flex items-center gap-3">
              ...
              {isLastStep ? (
                <Button
                  ...
                  className="bg-brand-green hover:bg-brand-green/90 text-white rounded-xl px-6"
                >
                  {isCreating ? (
                    <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ...
                <Button
                  ...
                  disabled={generatedTasks[currentBoard.id].length === 0}
