...
const VirtualAssistants = () => {
  ...
  const { pricing, loading: pricingLoading } = useVAPricing();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-5 p-4 sm:p-6 md:p-8 bg-card/50">
          <div className="flex items-center gap-3 sm:gap-5">
            <img
              ...
              onClick={() => navigate("/")}
              src="/lovable-uploads/3fb9c10e-bb47-4a0b-89c9-34a1a7b8dcd3.png"
            />
            <Button
              variant="ghost"
              size="icon"
          ...
          {user && (
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="rounded-xl hover:bg-secondary/80 transition-all duration-200 btn-smooth h-9 w-9 sm:h-10 sm:w-10"
        ...
        <div className="flex-1 p-4 sm:p-6 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {virtualAssistants.map((assistant) => {
              ...
              return (
                <Card key={assistant.id} className="relative flex flex-col hover:shadow-lg transition-shadow duration-200">
                  {assistant.popular && (
                    <Badge className="absolute -top-2 -right-2 bg-[#176884] text-white border-0">
                      Popular
                    </Badge>
                  )}
                  <CardHeader>
                      ...
                      pricing.map((pkg) => {
                        ...
                        return (
                          <Button
                            ...
                            onClick={() => handleCheckout(assistant.id, pkg.hours_package)}
                            disabled={isLoading || loadingCheckout !== null}
                            className="w-full justify-between bg-[#176884] hover:bg-[#176884]/90 text-white"
                            size="sm"
                          >
